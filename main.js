...
function renderTeams() {
  teamAEl.innerHTML = "";
  teamBEl.innerHTML = "";
  state.teamA.forEach((c, i) => teamAEl.appendChild(miniTemplate(c, "teamA", i)));
  state.teamB.forEach((c, i) => teamBEl.appendChild(miniTemplate(c, "teamB", i)));
}
...
function startBattle() {
  state.battle = {
    ...
    turn: "A" // fastest could start; simple rule: A starts
  };
...
function checkKO() {
  ...
  if (!state.battle.teamA.length || !state.battle.teamB.length) {
    ...
    turnInd.textContent = !state.battle.teamA.length ? "Vitória: Equipe B" : "Vitória: Equipe A";
    return true;
  }
  return false;
}
...
function getPositions() {
  ...
  const from = { x: aRect.right - layerRect.left, y: aRect.top - layerRect.top + aRect.height / 2 };
  const to = { x: bRect.left - layerRect.left, y: bRect.top - layerRect.top + bRect.height / 2 };
  return { from, to };
}
...
function animateMultiDots(dots, from, to, duration, cb) {
  ...
  function frame(t) {
    ...
    else { dots.forEach(d => d.remove()); if (cb) cb(); }
  }
  requestAnimationFrame(frame);
}
